'''Python 3 script to compress, archive and upload data files to Google Drive.  Usage:
...
try:
    ...
    for fn, bucket_key in upload_pending.copy():
        ...
        try:
            credentials_file = config.get("google-drive-key-file")
            ...
            export_fname = fn
            export_metadata = {'name':export_fname, 'parents':[config.get("google-drive-folder-id")]}
            export_media = MediaFileUpload(export_fname, mimetype='application/octet-stream', resumable=True)
